...
| `description` | No | markdown text | empty | assessment の説明。`assessment/presentation_material` に出力 |
...
| `question_layout` | No | enum (`I`, `S`, `A`) | `I` | assessment metadata `QUESTION_LAYOUT` |
...
| `description` | `assessment/presentation_material` |
...
`question_layout` の値は次の通りとする。

- `I`: 各問題は別のウェブページにあります
- `S`: 問題グループは別のウェブページにあります
- `A`: テスト全体が一画面で表示されます

- `description` は Markdown として解釈されなければならない。
- `allow_raw_html=false` の場合、`description` 中の raw HTML は入力検証エラーでなければならない。
